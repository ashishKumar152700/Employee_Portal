...
// Suggested searches: "location pulse", "fingerprint scan", "success check", "error cross"
const LOTTIE = {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <View style={[styles.overlayAccent, { backgroundColor: config.bgAccent }]}>
          {/* Phase label row */}
          <View style={styles.overlayPhaseRow}>
          ...
          <LottieView
            ...
            colorFilters={[
              {
                keypath: "**",          // tint all layers
                color: config.animColor,
              },
            ]}
          />
        </View>
        ...
        <View style={styles.overlayTextSection}>
          ...
          {/* Dot progress indicator (only during active phases) */}
          {(phase === "locating" || phase === "processing") && (
            <View style={styles.dotsRow}>
              {(["locating", "processing"] as PunchPhase[]).map((p) => (
                <View
                  key={p as string}
                  style={[
                    styles.dot,
                    phase === p ? styles.dotActive : styles.dotInactive,
                  ]}
                />
              ))}
            </View>
...
const PunchScreen: React.FC = () => {
    ...
    try {
      ...
      if (punch) {
        ...
        if (punch.duration != null) {
          setTotalTime(`${Math.floor(punch.duration / 60)}h ${punch.duration % 60}m`);
  ...
  const handleClockIn = async () => {
    ...
    try {
      ...
      const newPunchTime = response.data?.punchintime || new Date().toLocaleTimeString();
  ...
  const handleClockOut = async () => {
    ...
    if (!latestPunch?.punchintime) {
      Toast.show({ type: "error", text1: "Error", text2: "Please punch in first" });
      return;
    }
      ...
      if (duration == null && clockInTime) {
        duration = Math.round(
          (new Date(`1970-01-01T${newPunchOutTime}`).getTime() -
            new Date(`1970-01-01T${clockInTime}`).getTime()) / 60000,
        );
      }
      if (duration != null) {
        ...
        <View style={styles.modalContainer}>
          <View style={styles.mapContainer}>
            ...
            <TouchableOpacity
              ...
              onPress={() => { setIsMapVisible(false); setLocation(null); setAddress(""); }}
            >
